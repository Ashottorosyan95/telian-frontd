...
const AllCategory = () => {
    ...
    useEffect(() => {
      axios.get(process.env.REACT_APP_BACKEND_URL + '/category', {headers: {"Access-Control-Allow-Origin": "*"}}).then(res => {
        if(res.status === 200) {
      ...
      const deleteCategory = (id) => {
        axios.post(process.env.REACT_APP_BACKEND_URL + '/deleteCategory', {id: id}).then(res => {
          if(res.status === 200) {
            setCategories(res.data.categories)
          }
        })
                       ...
                       {/* <Link to="/vendor/add-products" data-toggle="tab" className="theme-btn-one bg-black btn_sm add_prod_button">Ավելացնել նոր ապրանք</Link> */}
                        ...
                        <div className="table-responsive">
                            <table className="table pending_table">
                                ...
                                <tbody>
                                    {categories.slice(0,100).map((data, index)=>(
                                        <tr key={index}>
                                            <td>{data.name}</td>
                                            <td><button style={{background:"Transparent"}} onClick={() => deleteCategory(data.id)} ><i className="fa fa-trash"></i></button></td>
                                        </tr>
                                    ))}
